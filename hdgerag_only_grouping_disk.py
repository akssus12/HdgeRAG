...
def sort_queries_by_clustering(edgeRagSearcher, messages, model):
    ...
    # Step 5: Print queries grouped by their assigned cluster
    for cluster_id, queries in sorted(cluster_groups.items()):
        print(f"\nCluster {cluster_id}:")
        for query, faiss_clusters in queries:
            print(f"  - {query} {sorted(faiss_clusters)}")  # Sort for readability
    '''

    # Step 4: Sort queries by cluster assignments
